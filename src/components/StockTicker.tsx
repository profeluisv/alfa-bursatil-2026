...
export function StockTicker() {
  ...
  return (
    <div className="w-full bg-black/95 border-b border-white/10 h-14 flex items-center sticky top-0 z-[100] overflow-hidden backdrop-blur-md">
      <div className="flex animate-marquee whitespace-nowrap">
        {[...data, ...data].map((item, i) => (
          <div key={i} className="inline-flex items-center px-10 space-x-3 border-r border-white/5 text-white uppercase tracking-wide">
            <span className="font-bold text-sm">{item.s}</span>
            <span className="font-mono text-base font-bold text-white">${item.p.toLocaleString('es-VE', {minimumFractionDigits:2})}</span>
            <span className={`text-sm font-bold ${item.v >= 0 ? 'text-emerald-400' : 'text-red-500'}`}>
